...
choices.forEach((choice) => {
  choice.addEventListener("click", () => {
    if(!isUserChoiceLocked){
      ...
      user.style.backgroundImage = `url(${classList}.png)`;
...
start.addEventListener("click", () => {
  if (user.style.backgroundImage !== "") {
    if (random > 0 && random < 0.33) {
      computer.style.backgroundImage = "url(stone.png)";
    } else if (random > 0.33 && random < 0.66) {
      computer.style.backgroundImage = "url(paper.png)";
    } else {
      computer.style.backgroundImage = "url(scissor.png)";
    }
    ...
    const userImg = user.style.backgroundImage.replace(/url\(["']?|["']?\)/g, "");
    const comImg = computer.style.backgroundImage.replace(/url\(["']?|["']?\)/g, "");
